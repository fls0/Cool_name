...
def main(): 
    ...
    while True:
        ...
        if  input_str.startswith("save_nuclear"):
            result =command_save('Map/coordinates_nuclear.txt','russia_map_nuclear.html')
            print(f"Карта з  прапорцями збережена у файлі {result}.")  
        elif input_str.startswith("save_air"):
            result = command_save('Map/coordinates_air.txt','russia_map_air.html')
            print(f"Карта з  прапорцями збережена у файлі {result}.") 
        elif input_str.startswith("save_admin"):
            result =command_save('Map/coordinates_admin.txt','russia_map_admin.html')
            print(f"Карта з  прапорцями збережена у файлі {result}.")   
        elif input_str == "add_nuclear":
            input_str = input("Приклад: 55.7558,37.6176. Введіть нові кординати:")
            print(add_coordinates('Map/coordinates_nuclear.txt', input_str))
        elif input_str == "add_air":
            input_str = input("Приклад: 55.7558,37.6176. Введіть нові кординати:")
            print(add_coordinates('Map/coordinates_air.txt', input_str))
        elif input_str == "add_admin":
            input_str = input("Приклад: 55.7558,37.6176. Введіть нові кординати:")
            print(add_coordinates('Map/coordinates_admin.txt', input_str))
